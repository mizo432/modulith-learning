 ...
 * <p>
 * This class manages the iteration state and provides helper methods for determining the next
...
public abstract class AbstractIterator<T> extends UnmodifiableIterator<T> {
   ...
   * <ul>
   *   <li>{@code READY}: The next element is available for retrieval.</li>
   *   <li>{@code NOT_READY}: The iterator has not attempted to compute the next element.</li>
   *   <li>{@code DONE}: The iteration has completed, and no more elements are available.</li>
   *   <li>{@code FAILED}: An error occurred during the computation of the next element.</li>
   * </ul>
   ...
   * <p>
   * This protected constructor ensures that the AbstractIterator class can only be instantiated as
   * part of its subclasses. It allows subclasses to manage and define their own specific iteration
   * logic by implementing required abstract methods.
   */
   ...
   * <p>
   * This method retrieves and returns the next available element from the iterator. If there are no
   * more elements to iterate over, it throws a {@code NoSuchElementException}. After retrieving the
   * next element, the state is updated, and the internal storage for the element is reset.
   *
   ...
   * <p>
   * This method allows a peek at the next element in the sequence without altering the internal
   * state of the iterator. If the iteration has no more elements, it throws a
   ...
   * <p>
   * The {@code State} enum is used to manage the lifecycle of the iteration process within the
   * {@code AbstractIterator} class. It defines the following states:
   *
   * <ul>
   *   <li>{@code READY}: Indicates that the next element in the iteration is ready to be returned.</li>
   *   <li>{@code NOT_READY}: Indicates that the next element is not yet computed, requiring further evaluation.</li>
   *   <li>{@code DONE}: Indicates that the iteration has reached its end, and no more elements are available.</li>
   *   <li>{@code FAILED}: Indicates that the iterator has encountered an error or unexpected condition.</li>
   * </ul>
   * <p>
   * The internal logic of {@code AbstractIterator} transitions between these states
   * to manage and determine the availability of the next element and the end of the iteration.
   */
  private enum State {
    /**
     * Indicates that the next element in the iteration is ready to be returned.
     * <p>
     * This state is used within the iteration process to signal that a subsequent element is
     ...
     * computation.
     * <p>
     * This state is used to show that the iterator is in the process of determining the next
     ...
     * <p>
     * The {@code DONE} state is used to signal that the iterator has exhausted all elements and the
     * iteration process is complete. This state marks the termination point of the iteration,
     ...
     * <p>
     * The {@code FAILED} state is used when an iteration process cannot be successfully completed
     * due to an exception or an invalid state. Once the iterator reaches this state, further
     * attempts to retrieve elements will not succeed. The specific cause of failure should be
     * handled elsewhere in the logic.
     */
    FAILED,
  }
}
