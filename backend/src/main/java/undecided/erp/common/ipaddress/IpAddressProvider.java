 ...
 *
 * <p>IPアドレスの取得には、java.netパッケージのInetAddressクラスを使用しています。
 *
 * <p>IpAddressProviderを使用するには、静的メソッド `ipAddress()` を呼び出すことで、IPアドレスを文字列として返します。
 *
 * <p>さらに高度な機能として、静的メソッド `setIpAddressProvider()`
 * を提供しており、これによってIpAddressProviderクラスのカスタム実装を設定することができます。デフォルトのIPアドレス取得メカニズムを異なる実装に置き換える場合に便利です。
 *
 * <p>IPアドレスプロバイダーをデフォルトの実装にリセットしたい場合は、静的メソッド `clear()` を呼び出します。
 *
 * <p>注意: IpAddressProviderクラスは、スレッドセーフを保証するためにAtomicReferenceを使用したシングルトンとして実装されています。
 */
public class IpAddressProvider {
   ...
   *
   * <p>デフォルトのIpAddressProviderのインスタンスを保持し、必要に応じて別のインスタンスに置き換えられます,.
   * この変数は、現在のIPアドレスプロバイダーの管理や切り替えに使用されます。
   *
   * <p>注意: この変数はスレッドセーフな操作のためにAtomicReferenceを使用しています。
   */
  private static final AtomicReference<IpAddressProvider> ipAddressProvider =
      new AtomicReference<>(new IpAddressProvider());

  /**
   * IpAddressProviderクラスのデフォルトコンストラクタ。
   *
   * <p>ホストマシンの現在のIPアドレスを提供するためのデフォルトインスタンスを生成します。 通常、このコンストラクタは直接使用されることはなく、
   * スタティックメソッドやサブクラスを通して利用されます。
   *
   * <p>注意: このクラスはスレッドセーフに設計されており、AtomicReferenceを使用して デフォルトインスタンスを管理します。
   */
  IpAddressProvider() {}
   ...
   *
   * <p>このコンストラクタにより、新しいIpAddressProviderインスタンスで 現在のIPアドレスプロバイダーを設定します。
   *
  ...
  public static String ipAddress() throws UnknownHostException {
    return IpAddressProvider.ipAddressProvider.get().ipHostAddress();
  }

  /**
   * 現在のIPアドレスプロバイダーを設定します。
   *
   * <p>このメソッドを使用して、カスタムのIpAddressProviderを設定することができます。 設定後、{@link #ipAddress()}
