 ...
 *
 * <ul>
 *   <li>`backlogId`: プロダクトバックログ固有の識別子を表す。SnowflakeIdを使用。
 *   <li>`product`: このバックログが属するプロダクト。
 *   <li>`name`: プロダクトバックログの名前。
 *   <li>`description`: プロダクトバックログの詳細説明。
 * </ul>
 */
@AllArgsConstructor
@Entity
...
public class ProductBacklog extends PptEntity<ProductBacklog> implements Serializable {
   ...
   *
   * <p>アプリケーションにおける各プロダクトバックログを一意に識別するために使用されます。 データベース上の "backlog_id" カラムに対応し、null 値は許可されていません。
   */
  @Id
   ...
   *
   * <p>プロダクトとプロダクトバックログの関連付けを表します。 データベース上の "product_id" カラムに対応し、null 値は許可されていません。
   */
  @ManyToOne
   ...
   * プロダクトバックログの名前を表す変数。
   *
   * <p>プロダクトバックログの識別に使用される名前です。 データベース上の "name" カラムに対応し、null 値は許可されていません。
   */
  @Getter
   ...
   *
   * <p>プロダクトバックログの詳細な説明や背景情報を提供します。 データベース上の "description" カラムに対応します。
   */
  @Getter
