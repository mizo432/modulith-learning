...
public class DepartmentCommandImpl implements DepartmentCommand {
    ...
    public void update(DepartmentCode departmentCode, Department department) {
        Optional<Department> oldOption = repository.findByDepartmentCode(departmentCode);
        Department oldDepartment = oldOption.orElseThrow(() -> new EntityNotFoundException("Department not found: " + departmentCode));
