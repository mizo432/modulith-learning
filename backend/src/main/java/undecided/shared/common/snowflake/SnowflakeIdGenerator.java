...
public class SnowflakeIdGenerator {
   ...
   *
   * <p>この定数は1609459200000Lに設定されており、これは2021年1月1日の00:00:00 UTCをミリ秒です。
   *
   * <p>この変数は{@link
   ...
   *
   * <p>この値はIDが生成されるたびにインクリメントされ、現在のタイムスタンプが変更されると0にリセットされます。 これにより、同じミリ秒内に生成されたIDが一意であることが保証されます。
   */
   ...
   *
   * <p>初期値は-62167252739000Lです。 IDが生成されるたびにlastTimestampの値は更新されます。
   */
  ...
  public SnowflakeIdGenerator(Long workerId) {
    if (workerId < 0 || workerId > MAX_WORKER_ID) {
      throw new IllegalArgumentException(
          "worker ID must be between 0 and " + MAX_WORKER_ID + " but workerId is " + workerId);
    }
