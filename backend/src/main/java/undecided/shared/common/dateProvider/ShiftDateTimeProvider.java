...
public class ShiftDateTimeProvider extends DateProvider {
   ...
   *
   * <p>This variable is used to calculate the time shift applied to the current LocalDateTime
   * within the ShiftDateTimeProvider class.
   *
   * <p>The START_DATE_TIME is initially set to the current LocalDateTime at the moment an instance
   * of ShiftDateTimeProvider is created. Subsequent calculations for accessing shifted current time
   * rely on the difference between this start time and the actual system current time.
   *
   * <p>Use this variable to track and compute time shifts in scenarios where the apparent local
   * time needs to be advanced or delayed systematically for testing or scheduling purposes.
   */
   ...
   *
   * <p>This variable holds the base LocalDateTime value from which any time shifts are calculated,
   * allowing the class to simulate different current times based on the intended time manipulation
   * logic.
   *
   * <p>LOCAL_DATE_TIME is initially set during the construction of a ShiftDateTimeProvider
   * instance. It is updated to represent the "current" LocalDateTime as determined by the internal
   * logic when calculating the shifted time in correlation with the real system time.
   */
