...
public class ObjectPrecondition {
  ...
  public static <T> T checkNotNull(
      T refer, @NonNull Supplier<? extends RuntimeException> supplier) {
    if (refer == null) {
      throw supplier.get();
    }
    return refer;
  ...
  public static void checkArgument(
      boolean expression, @NonNull Supplier<? extends RuntimeException> supplier) {
    if (!expression) {
      throw supplier.get();
    }
  }
   ...
   */
  public static void checkArgument(boolean expression, @NonNull String label) {
    if (!expression) {
   ...
   *
   * <p>もし式がfalseの場合、エラーメッセージを含むIllegalStateExceptionがスローされます。
   *
