 ...
 *
 * <p>引数として {@link Boolean} 型を受け取り、値が {@code true} または {@code false} であることを確認し、
 * 必要に応じて例外をスローします。特に、メソッドレベルでの検証や安全性のチェックに適しています。
 *
 * <p>このクラスは、条件付きの検証や防御的プログラミングに役立ちます。 例えば、特定の条件が満たされない場合に例外をスローし、プログラムの安全性を確保します。
 *
...
public class BooleanPrecondition {
   ...
   *
   * <p>参照が {@code null} の場合、{@code null} を返します。 参照が {@code true} の場合、{@code true} を返します。 参照が {@code
   * false} の場合、exceptionSupplierによって提供される例外をスローします。
   *
   ...
   */
  public static Boolean checkTrue(
      Boolean ref, @NonNull Supplier<? extends RuntimeException> exceptionSupplier) {
    if (ref == null) {
      return null;
    }
    if (ref) {
   ...
   *
   * <p>参照が {@code null} の場合、{@code null} を返します。 参照が {@code false} の場合、{@code false} を返します。 参照が
